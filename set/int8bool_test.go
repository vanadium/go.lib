...
func TestInt8Bool(t *testing.T) {
	...
	{
		s1 := Int8Bool.FromSlice(slice)
		s2 := Int8Bool.FromSlice(slice[1:])
		Int8Bool.Difference(s1, s2)
		...
		s3 := Int8Bool.FromSlice(slice[1:])
		s4 := Int8Bool.FromSlice(slice)
		Int8Bool.Difference(s3, s4)
		...
		s5 := Int8Bool.FromSlice(slice[1:])
		s6 := Int8Bool.FromSlice(slice[:1])
		Int8Bool.Difference(s5, s6)
		...
		s7 := Int8Bool.FromSlice(slice)
		s8 := Int8Bool.FromSlice(slice)
		Int8Bool.Difference(s7, s8)
		for i, want := range []bool{false, false} {
	...
	// Test set union.
	{
		s1 := Int8Bool.FromSlice(slice[:1])
		s2 := Int8Bool.FromSlice(slice[1:])
		Int8Bool.Union(s1, s2)
		for i, want := range []bool{true, true} {
			if _, got := s1[(slice[i])]; got != want {
				t.Errorf("index %d: got %v, want %v", i, got, want)
			}
