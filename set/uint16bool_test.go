...
func TestUint16Bool(t *testing.T) {
	slice := []uint16{}
	...
	{
		s1 := Uint16Bool.FromSlice(slice)
		s2 := Uint16Bool.FromSlice(slice[1:])
		Uint16Bool.Difference(s1, s2)
		...
		s3 := Uint16Bool.FromSlice(slice[1:])
		s4 := Uint16Bool.FromSlice(slice)
		Uint16Bool.Difference(s3, s4)
		...
		s5 := Uint16Bool.FromSlice(slice[1:])
		s6 := Uint16Bool.FromSlice(slice[:1])
		Uint16Bool.Difference(s5, s6)
		...
		s7 := Uint16Bool.FromSlice(slice)
		s8 := Uint16Bool.FromSlice(slice)
		Uint16Bool.Difference(s7, s8)
		for i, want := range []bool{false, false} {
	...
	// Test set union.
	{
		s1 := Uint16Bool.FromSlice(slice[:1])
		s2 := Uint16Bool.FromSlice(slice[1:])
		Uint16Bool.Union(s1, s2)
		for i, want := range []bool{true, true} {
			if _, got := s1[(slice[i])]; got != want {
				t.Errorf("index %d: got %v, want %v", i, got, want)
			}
