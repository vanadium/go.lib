...
func TestUint(t *testing.T) {
	...
	{
		s1 := Uint.FromSlice(slice)
		s2 := Uint.FromSlice(slice[1:])
		Uint.Difference(s1, s2)
		...
		s3 := Uint.FromSlice(slice[1:])
		s4 := Uint.FromSlice(slice)
		Uint.Difference(s3, s4)
		...
		s5 := Uint.FromSlice(slice[1:])
		s6 := Uint.FromSlice(slice[:1])
		Uint.Difference(s5, s6)
		...
		s7 := Uint.FromSlice(slice)
		s8 := Uint.FromSlice(slice)
		Uint.Difference(s7, s8)
		for i, want := range []bool{false, false} {
