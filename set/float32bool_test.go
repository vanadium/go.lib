...
func TestFloat32Bool(t *testing.T) {
	...
	s1 := Float32Bool.FromSlice(slice)
	...
	{
		s1 := Float32Bool.FromSlice(slice)
		s2 := Float32Bool.FromSlice(slice[1:])
		Float32Bool.Difference(s1, s2)
		...
		s3 := Float32Bool.FromSlice(slice[1:])
		s4 := Float32Bool.FromSlice(slice)
		Float32Bool.Difference(s3, s4)
		...
		s5 := Float32Bool.FromSlice(slice[1:])
		s6 := Float32Bool.FromSlice(slice[:1])
		Float32Bool.Difference(s5, s6)
		...
		s7 := Float32Bool.FromSlice(slice)
		s8 := Float32Bool.FromSlice(slice)
		Float32Bool.Difference(s7, s8)
		for i, want := range []bool{false, false} {
	...
	// Test set intersection.
	{
		s1 := Float32Bool.FromSlice(slice)
		s2 := Float32Bool.FromSlice(slice[1:])
		Float32Bool.Intersection(s1, s2)
		for i, want := range []bool{false, true} {
			if _, got := s1[(slice[i])]; got != want {
				t.Errorf("index %d: got %v, want %v", i, got, want)
			}
		...
		s3 := Float32Bool.FromSlice(slice[1:])
		s4 := Float32Bool.FromSlice(slice)
		Float32Bool.Intersection(s3, s4)
		for i, want := range []bool{false, true} {
			if _, got := s3[(slice[i])]; got != want {
				t.Errorf("index %d: got %v, want %v", i, got, want)
			}
		...
		s5 := Float32Bool.FromSlice(slice[1:])
		s6 := Float32Bool.FromSlice(slice[:1])
		Float32Bool.Intersection(s5, s6)
		for i, want := range []bool{false, false} {
			if _, got := s5[(slice[i])]; got != want {
				t.Errorf("index %d: got %v, want %v", i, got, want)
			}
		...
		s7 := Float32Bool.FromSlice(slice)
		s8 := Float32Bool.FromSlice(slice)
		Float32Bool.Intersection(s7, s8)
		for i, want := range []bool{true, true} {
			if _, got := s7[(slice[i])]; got != want {
				t.Errorf("index %d: got %v, want %v", i, got, want)
			}
	...
	// Test set union.
	{
		s1 := Float32Bool.FromSlice(slice[:1])
		s2 := Float32Bool.FromSlice(slice[1:])
		Float32Bool.Union(s1, s2)
		for i, want := range []bool{true, true} {
			if _, got := s1[(slice[i])]; got != want {
				t.Errorf("index %d: got %v, want %v", i, got, want)
			}
